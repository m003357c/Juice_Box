	...
	$(".pay-screen .btn").click(function(){		
		...
		setTimeout(function(){
			if ($(".card.animated").hasClass("mobile")){ 
				...
				$(".card.debit").addClass("card-animate-in card-animate-out").delay(1000).queue(function() { $(".card.debit").removeClass("card-animate-out").css("z-index","2"); } );	

			} else if ($(".card").hasClass("paypal")){

				$(".card.paypal").addClass("paypal-animate-in paypal-animate-out").delay(1000).queue(function() { $(".card.paypal").removeClass("paypal-animate-out").css("z-index","3"); } );	
